...
int main(int argc, char **argv) {
	...
	static dcomplex truth[NZ][NY][NX];
	...
	compute_error<dcomplex>(&u1[0][0][0], &truth[0][0][0], NZ*NY*NX);
	...
	compute_error<dcomplex, qcomplex>(&u0[0][0][0], &timestep_truth[0][0][0], NZ*NY*NX);
	...
	for (iter = 1; iter <= niter; iter++) {
		...
		if(iter==1) {
			...
			compute_error<dcomplex, qcomplex>(&u1[0][0][0], &timestep_truth[0][0][0], NZ*NY*NX);
		}
		if (TIMERS_ENABLED == TRUE) {  
			timer_start(T_FFT);
		}
		...
		if(iter==1) {
			...
			compute_error<dcomplex, qcomplex>(&u2[0][0][0], &timestep_truth[0][0][0], NZ*NY*NX);
		}
	}
	...
	c_print_results("FT", problem_class, NX, NY, NZ, niter, nthreads,
			total_time, mflops, "   double floating point", verified, 
