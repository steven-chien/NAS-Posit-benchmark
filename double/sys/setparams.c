/* 
 * This utility configures a NPB to be built for a specific problem_class. 
 ...
 * the number of nodes and problem_class for which a benchmark is being built. 

 * The utility takes 3 arguments: 
 *       setparams benchmark-name problem_class
 *    benchmark-name is "sp", "bt", etc
 *    problem_class is the size of the benchmark
 ...
 * 
 * This program makes use of two extra benchmark "problem_classes"
 * problem_class "X" means an invalid specification. It is returned if
 * there is an error parsing the config file. 
 * problem_class "U" is an external specification meaning "unknown problem_class"
 * 
...
void get_info(char *argv[], int *typep, char *problem_classp);
void check_info(int type, char problem_class);
void read_info(int type, char *problem_classp);
void write_info(int type, char problem_class);
void write_sp_info(FILE *fp, char problem_class);
void write_bt_info(FILE *fp, char problem_class);
void write_lu_info(FILE *fp, char problem_class);
void write_mg_info(FILE *fp, char problem_class);
void write_cg_info(FILE *fp, char problem_class);
void write_ft_info(FILE *fp, char problem_class);
void write_ep_info(FILE *fp, char problem_class);
void write_is_info(FILE *fp, char problem_class);
...
{
  int type;
  char problem_class, problem_class_old;
  
  if (argc != 3) {
    printf("Usage: %s benchmark-name problem_class\n", argv[0]);
    exit(1);
  }

  /* Get command line arguments. Make sure they're ok. */
  get_info(argv, &type, &problem_class);
  if (problem_class != 'U') {
#ifdef VERBOSE
    printf("setparams: For benchmark %s: problem_class = %c\n", 
	   argv[1], problem_class); 
#endif
    check_info(type, problem_class);
  }

  /* Get old information. */
  read_info(type, &problem_class_old);
  if (problem_class != 'U') {
    if (problem_class_old != 'X') {
#ifdef VERBOSE
      printf("setparams:     old settings: problem_class = %c\n", 
	     problem_class_old); 
#endif
    }
  } else {
  ...
  * For example, to build a problem_class A benchmark, type                   *\n\
  ...
  *********************************************************************\n\n"); 
    ...
    exit(1); /* exit on problem_class==U */
  }

  /* Write out new information if it's different. */
  if (problem_class != problem_class_old) {
#ifdef VERBOSE
    printf("setparams: Writing %s\n", FILENAME); 
#endif
    write_info(type, problem_class);
  } else {
#ifdef VERBOSE
...
void check_info(int type, char problem_class) 
{
  ...
  if (problem_class != 'S' && 
      ...
      problem_class != 'W' && 
      problem_class != 'C') {
    printf("setparams: Unknown benchmark problem_class %c\n", problem_class); 
    printf("setparams: Allowed problem_classes are \"S\", \"A\", \"B\" and \"C\"\n");
    exit(1);
  }
...
void read_info(int type, char *problem_classp)
{
  ...
  switch(type) {
      ...
      case EP:
      case CG:
          nread = fscanf(fp, DESC_LINE, problem_classp);
          if (nread != 1) {
          ...
          break;
      case IS:
          nread = fscanf(fp, DEF_CLASS_LINE, problem_classp);
          if (nread != 1) {
            printf("setparams: Error parsing config file %s. Ignoring previous settings\n", FILENAME);
            goto abort;
          }
...
void write_info(int type, char problem_class) 
{
  ...
  switch(type) {
      ...
      case CG:
          ...
          fprintf(fp, DESC_LINE, problem_class);
...
*/\n");
          ...
          break;
      case IS:
          fprintf(fp, DEF_CLASS_LINE, problem_class);
          fprintf(fp, "\
/*\n\
   This file is generated automatically by the setparams utility.\n\
   It sets the number of processors and the problem_class of the NPB\n\
  ...
  switch(type) {
  case SP:
    write_sp_info(fp, problem_class);
    break;	      
  case BT:	      
    write_bt_info(fp, problem_class);
    break;	      
  case LU:	      
    write_lu_info(fp, problem_class);
    break;	      
  case MG:	      
    write_mg_info(fp, problem_class);
    break;	      
  case IS:	      
    write_is_info(fp, problem_class);  
    break;	      
  case FT:	      
    write_ft_info(fp, problem_class);
    break;	      
  case EP:	      
    write_ep_info(fp, problem_class);
    break;	      
  case CG:	      
    write_cg_info(fp, problem_class);
    break;
  default:
    printf("setparams: (Internal error): Unknown benchmark type %d\n", type);
    exit(1);
...
void write_sp_info(FILE *fp, char problem_class) 
{
  ...
  char *dt;
  if      (problem_class == 'S') { problem_size = 12;  dt = "0.015";   niter = 100; }
  else if (problem_class == 'W') { problem_size = 36;  dt = "0.0015";  niter = 400; }
  else if (problem_class == 'A') { problem_size = 64;  dt = "0.0015";  niter = 400; }
  else if (problem_class == 'B') { problem_size = 102; dt = "0.001";   niter = 400; }
  else if (problem_class == 'C') { problem_size = 162; dt = "0.00067"; niter = 400; }
  else {
    printf("setparams: Internal error: invalid problem_class %c\n", problem_class);
    exit(1);
  }
...
void write_bt_info(FILE *fp, char problem_class) 
{
  int problem_size, niter;
  char *dt;
  if      (problem_class == 'S') { problem_size = 12;  dt = "0.010";   niter = 60; }
  else if (problem_class == 'W') { problem_size = 24;  dt = "0.0008";  niter = 200; }
  else if (problem_class == 'A') { problem_size = 64;  dt = "0.0008";  niter = 200; }
  else if (problem_class == 'B') { problem_size = 102; dt = "0.0003";  niter = 200; }
  else if (problem_class == 'C') { problem_size = 162; dt = "0.0001";  niter = 200; }
  else {
    printf("setparams: Internal error: invalid problem_class %c\n", problem_class);
    exit(1);
  }
...
void write_lu_info(FILE *fp, char problem_class) 
{
  ...
  if      (problem_class == 'S') { problem_size = 12;  dt_default = "0.5"; itmax = 50; }
  else if (problem_class == 'W') { problem_size = 33;  dt_default = "1.5e-3"; itmax = 300; }
  else if (problem_class == 'A') { problem_size = 64;  dt_default = "2.0"; itmax = 250; }
  else if (problem_class == 'B') { problem_size = 102; dt_default = "2.0"; itmax = 250; }
  else if (problem_class == 'C') { problem_size = 162; dt_default = "2.0"; itmax = 250; }
  else {
    printf("setparams: Internal error: invalid problem_class %c\n", problem_class);
    exit(1);
  }
...
void write_mg_info(FILE *fp, char problem_class) 
{
  ...
  if      (problem_class == 'S') { problem_size = 32; nit = 4; }
  else if (problem_class == 'W') { problem_size = 64; nit = 40; }
  else if (problem_class == 'A') { problem_size = 256; nit = 4; }
  else if (problem_class == 'B') { problem_size = 256; nit = 20; }
  else if (problem_class == 'C') { problem_size = 512; nit = 20; }
  else {
    printf("setparams: Internal error: invalid problem_class type %c\n", problem_class);
    exit(1);
  }
...
void write_ft_info(FILE *fp, char problem_class) 
{
  ...
  if      (problem_class == 'S') { nx = 64; ny = 64; nz = 64; niter = 6;}
  else if (problem_class == 'W') { nx = 128; ny = 128; nz = 32; niter = 6;}
  else if (problem_class == 'A') { nx = 256; ny = 256; nz = 128; niter = 6;}
  else if (problem_class == 'B') { nx = 512; ny = 256; nz = 256; niter =20;}
  else if (problem_class == 'C') { nx = 512; ny = 512; nz = 512; niter =20;}
  else {
    printf("setparams: Internal error: invalid problem_class type %c\n", problem_class);
    exit(1);
  }
...
void write_ep_info(FILE *fp, char problem_class)
{
   ...
   */
  int m;
  if      (problem_class == 'S') { m = 24; }
  else if (problem_class == 'W') { m = 25; }
  else if (problem_class == 'A') { m = 28; }
  else if (problem_class == 'B') { m = 30; }
  else if (problem_class == 'C') { m = 32; }
  else {
    printf("setparams: Internal error: invalid problem_class type %c\n", problem_class);
    exit(1);
  }
