...
static void error_norm(float rms[5]);
static void error_norm(__float128 rms[5]);
static void rhs_norm(float rms[5]);
static void rhs_norm_quad(__float128 rms[5]);
static void exact_rhs(void);
static void exact_solution(float xi, float eta, float zeta,
		float dtemp[5]);
...
static void verify(int no_time_steps, char *problem_classs, boolean *verified);
static void x_solve(void);
static void x_backsubstitute(void);
static void x_solve_cell(void);
static void matvec_sub(float ablock[5][5], float avec[5], float bvec[5]);
static void matmul_sub(float ablock[5][5], float bblock[5][5],
		float cblock[5][5]);
static void binvcrhs(float lhs[5][5], float c[5][5], float r[5]);
static void binvrhs(float lhs[5][5], float r[5]);
...
int main(int argc, char **argv) {
	...
	for (step = 1; step <= niter; step++) {
		...
		rhs_norm_quad(xcr);
		for(int m=0; m<5;m++) {
			xcr[m] = xcr[m] / static_cast<__float128>(dt);
		}
	...
	verify(niter, &problem_classs, &verified);
	...
	c_print_results("BT", problem_classs, grid_points[0], 
...
static void rhs_norm_quad(__float128 rms[5]) {
	...
	for (i = 1; i < grid_points[0]-1; i++) {
		for (j = 1; j < grid_points[1]-1; j++) {
			for (k = 1; k < grid_points[2]-1; k++) {
				for (m = 0; m < 5; m++) {
					add = static_cast<__float128>(rhs[i][j][k][m]);
					rms[m] = rms[m] + add*add;
				}
			}
		}
...
static void exact_rhs(void) {
	...
	float dtemp[5], xi, eta, zeta, dtpp;
...
static void exact_solution(__float128 xi, __float128 eta, __float128 zeta,
	...
	for (m = 0; m < 5; m++) {
		dtemp[m] =  (__float128)ce[m][0] +
			xi*((__float128)ce[m][1] + xi*((__float128)ce[m][4] + xi*((__float128)ce[m][7]
							+ xi*(__float128)ce[m][10]))) +
			eta*((__float128)ce[m][2] + eta*((__float128)ce[m][5] + eta*((__float128)ce[m][8]
							+ eta*(__float128)ce[m][11])))+
			zeta*((__float128)ce[m][3] + zeta*((__float128)ce[m][6] + zeta*((__float128)ce[m][9] + 
							zeta*(__float128)ce[m][12])));
	}
}
	...
	for (i = 0; i < grid_points[0]; i++) {
		...
		for (j = 0; j < grid_points[1]; j++) {
			...
			for (k = 0; k < grid_points[2]; k++) {
				...
				for (ix = 0; ix < 2; ix++) {
					exact_solution((float)ix, eta, zeta, 
							&(Pface[ix][0][0]));
				}

				for (iy = 0; iy < 2; iy++) {
					exact_solution(xi, (float)iy , zeta, 
							&Pface[iy][1][0]);
				}

				for (iz = 0; iz < 2; iz++) {
					exact_solution(xi, eta, (float)iz,   
							&Pface[iz][2][0]);
				}
...
static void verify(int no_time_steps, char *problem_classs, boolean *verified) {
	...
	*verified = TRUE;
	...
	if (*problem_classs != 'U') {
		printf(" Verification being performed for problem_classs %1c\n", *problem_classs);
