...
static void conj_grad (int colidx[], int rowstr[], float x[], float z[],
		float a[], float p[], float q[], float r[],
		//double w[],
		float *rnorm);
static void makea(int n, int nz, float a[], int colidx[], int rowstr[],
		int nonzer, int firstrow, int lastrow, int firstcol,
		int lastcol, float rcond, int arow[], int acol[],
		float aelt[], float v[], int iv[], float shift );
static void sparse(float a[], int colidx[], int rowstr[], int n,
		int arow[], int acol[], float aelt[],
		int firstrow, int lastrow,
		float x[], boolean mark[], int nzloc[], int nnza);
...
int main(int argc, char **argv) {
	...
	for (it = 1; it <= NITER; it++) {
		...
		printf("    %5d       %20.14e%20.13e%20.13Qe\n", it, rnorm, zeta, fabsq((__float128)zeta-(__float128)zeta_verify_value));
  ...
  c---------------------------------------------------------------------*/
{
	...
	float d, sum, rho, rho0, alpha, beta;
	int i, j, k;
	int cgit, cgitmax = 25;
	...
	for (cgit = 1; cgit <= cgitmax; cgit++) {
		...
		if(callcount>25) {
			compute_residual<float>(callcount-25, a, z, x, NA, firstrow, lastrow, firstcol, lastcol, rowstr, colidx);
		}
