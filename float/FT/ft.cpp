...
static void evolve(fcomplex u0[NZ][NY][NX], fcomplex u1[NZ][NY][NX],
		int t, int indexmap[NZ][NY][NX], int d[3]);
static void compute_initial_conditions(fcomplex u0[NZ][NY][NX], int d[3]);
...
static void fft(int dir, fcomplex x1[NZ][NY][NX], fcomplex x2[NZ][NY][NX]);
static void cffts1(int is, int d[3], fcomplex x[NZ][NY][NX],
		fcomplex xout[NZ][NY][NX],
		fcomplex y0[NX][FFTBLOCKPAD],
		fcomplex y1[NX][FFTBLOCKPAD]);
static void cffts2(int is, int d[3], fcomplex x[NZ][NY][NX],
		fcomplex xout[NZ][NY][NX],
		fcomplex y0[NX][FFTBLOCKPAD],
		fcomplex y1[NX][FFTBLOCKPAD]);
static void cffts3(int is, int d[3], fcomplex x[NZ][NY][NX],
		fcomplex xout[NZ][NY][NX],
		fcomplex y0[NX][FFTBLOCKPAD],
		fcomplex y1[NX][FFTBLOCKPAD]);
static void fft_init (int n);
static void cfftz (int is, int m, int n, fcomplex x[NX][FFTBLOCKPAD],
		fcomplex y[NX][FFTBLOCKPAD]);
static void fftz2 (int is, int l, int m, int n, int ny, int ny1,
		fcomplex u[NX], fcomplex x[NX][FFTBLOCKPAD],
		fcomplex y[NX][FFTBLOCKPAD]);
static int ilog2(int n);
static void checksum(int i, fcomplex u1[NZ][NY][NX], int d[3]);
...
int main(int argc, char **argv) {
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
	static fcomplex u0[NZ][NY][NX];
	static fcomplex pad1[3];
	static fcomplex u1[NZ][NY][NX];
	static fcomplex pad2[3];
	static fcomplex u2[NZ][NY][NX];
	static fcomplex pad3[3];
	...
	static fcomplex truth[NZ][NY][NX];
	...
	compute_error<fcomplex>(&u1[0][0][0], &truth[0][0][0], NZ*NY*NX);
	...
	compute_error<fcomplex, qcomplex>(&u0[0][0][0], &timestep_truth[0][0][0], NZ*NY*NX);
	...
	for (iter = 1; iter <= niter; iter++) {
		...
		if(iter==1) {
			...
			compute_error<fcomplex, qcomplex>(&u1[0][0][0], &timestep_truth[0][0][0], NZ*NY*NX);
		}
		if (TIMERS_ENABLED == TRUE) {  
			timer_start(T_FFT);
		}
		...
		if(iter==1) {
			...
			compute_error<fcomplex, qcomplex>(&u2[0][0][0], &timestep_truth[0][0][0], NZ*NY*NX);
		}
	}
	...
	c_print_results("FT", problem_class, NX, NY, NZ, niter, nthreads,
			total_time, mflops, "    float floating point", verified, 
...
static void checksum(int i, fcomplex u1[NZ][NY][NX], int d[3]) {
	...
	sums[i].real += (double)chk.real;
	sums[i].imag += (double)chk.imag;
	/* complex % real */
