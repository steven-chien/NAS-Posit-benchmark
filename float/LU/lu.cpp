...
static void blts (int nx, int ny, int nz, int k,
		float omega,
		float v[ISIZ1][ISIZ2/2*2+1][ISIZ3/2*2+1][5],
		float ldz[ISIZ1][ISIZ2][5][5],
		float ldy[ISIZ1][ISIZ2][5][5],
		float ldx[ISIZ1][ISIZ2][5][5],
		float d[ISIZ1][ISIZ2][5][5],
		int ist, int iend, int jst, int jend,
		int nx0, int ny0 );
static void buts(int nx, int ny, int nz, int k,
		float omega,
		float v[ISIZ1][ISIZ2/2*2+1][ISIZ3/2*2+1][5],
		float tv[ISIZ1][ISIZ2][5],
		float d[ISIZ1][ISIZ2][5][5],
		float udx[ISIZ1][ISIZ2][5][5],
		float udy[ISIZ1][ISIZ2][5][5],
		float udz[ISIZ1][ISIZ2][5][5],
		int ist, int iend, int jst, int jend,
		int nx0, int ny0 );
static void domain(void);
static void erhs(void);
static void error(void);
static void exact( int i, int j, int k, float u000ijk[5] );
...
static void verify(float xcr[5], __float128 xce[5], float xci,
...
static void erhs(void) {
	...
	float  u21j, u31j, u41j, u51j;
	float  u21k, u31k, u41k, u51k;
	float  u21im1, u31im1, u41im1, u51im1;
	float  u21jm1, u31jm1, u41jm1, u51jm1;
	float  u21km1, u31km1, u41km1, u51km1;
	...
	for (i = 0; i < nx; i++) {
		iglob = i;
		xi = ( (float)(iglob) ) / ( nx0 - 1 );
		for (j = 0; j < ny; j++) {
			jglob = j;
			eta = ( (float)(jglob) ) / ( ny0 - 1 );
			for (k = 0; k < nz; k++) {
				zeta = ( (float)(k) ) / ( nz - 1 );
				for (m = 0; m < 5; m++) {
					rsd[i][j][k][m] =  ce[m][0]
	...
	for (i = ist; i <= iend; i++) {
		...
		for (j = jst; j <= jend; j++) {
			...
			for (k = 1; k <= nz-2; k++) {
				...
				for (m = 0; m < 5; m++) {
					tmp = ( u000ijk[m] - (__float128)u[i][j][k][m] );
...
static void exact( int i, int j, int k, __float128 u000ijk[5] ) {
	...
	for (m = 0; m < 5; m++) {
		u000ijk[m] =  (__float128)ce[m][0]
			+ (__float128)ce[m][1] * xi
			+ (__float128)ce[m][2] * eta
			+ (__float128)ce[m][3] * zeta
			+ (__float128)ce[m][4] * xi * xi
			+ (__float128)ce[m][5] * eta * eta
			+ (__float128)ce[m][6] * zeta * zeta
			+ (__float128)ce[m][7] * xi * xi * xi
			+ (__float128)ce[m][8] * eta * eta * eta
			+ (__float128)ce[m][9] * zeta * zeta * zeta
			+ (__float128)ce[m][10] * xi * xi * xi * xi
			+ (__float128)ce[m][11] * eta * eta * eta * eta
			+ (__float128)ce[m][12] * zeta * zeta * zeta * zeta;
	}
}
...
static void l2norm_quad (int nx0, int ny0, int nz0,
	...
	for (i = ist; i <= iend; i++) {
		for (j = jst; j <= jend; j++) {
			for (k = 1; k <= nz0-2; k++) {
				sum0 = sum0 + static_cast<__float128>(v[i][j][k][0]) * static_cast<__float128>(v[i][j][k][0]);
				sum1 = sum1 + static_cast<__float128>(v[i][j][k][1]) * static_cast<__float128>(v[i][j][k][1]);
				sum2 = sum2 + static_cast<__float128>(v[i][j][k][2]) * static_cast<__float128>(v[i][j][k][2]);
				sum3 = sum3 + static_cast<__float128>(v[i][j][k][3]) * static_cast<__float128>(v[i][j][k][3]);
				sum4 = sum4 + static_cast<__float128>(v[i][j][k][4]) * static_cast<__float128>(v[i][j][k][4]);
			}
		}
	}
...
static void l2norm (int nx0, int ny0, int nz0,
	...
	/*--------------------------------------------------------------------
	  ...
	  --------------------------------------------------------------------*/
	int i, j, k, m;
	float sum0=0.0, sum1=0.0, sum2=0.0, sum3=0.0, sum4=0.0;
...
static void pintgr(void) {
	...
	float phi1[ISIZ2+2][ISIZ3+2];	/* phi1(0:isiz2+1,0:isiz3+1) */
	float phi2[ISIZ2+2][ISIZ3+2];	/* phi2(0:isiz2+1,0:isiz3+1) */
	float  frc1, frc2, frc3;
...
static void setiv(void) {
	...
	int iglob, jglob;
	float  xi, eta, zeta;
	float  pxi, peta, pzeta;
	float  ue_1jk[5],ue_nx0jk[5],ue_i1k[5],
...
static void ssor(void) {
	...
	int istep;
	float  tmp;
	float  delunm[5], tv[ISIZ1][ISIZ2][5];
...
static void verify(float xcr[5], __float128 xce[5], float xci,
	...
	*verified = TRUE;
	...
	for (m = 0; m < 5; m++) {
		...
		xcrdif[m] = fabsq(((__float128)xcr[m]-xcrref[m])/xcrref[m]);
		xcedif[m] = fabsq(((__float128)xce[m]-xceref[m])/xceref[m]);

	}
	xcidif = fabsq(((__float128)xci - xciref)/xciref);
	...
	if (*problem_class != 'U') {
		...
		if (fabsq((__float128)dt-dtref) > epsilon) {  
			*verified = FALSE;
			*problem_class = 'U';
			quadmath_snprintf(quad_str[0], sizeof quad_str[0], "%15.8Qe", dtref);
	...
	for (m = 0; m < 5; m++) {
		quadmath_snprintf(quad_str[1], sizeof quad_str[1], "%20.13Qe", xcrref[m]);
		quadmath_snprintf(quad_str[2], sizeof quad_str[2], "%20.13Qe", xcrdif[m]);
		...
		if (*problem_class  ==  'U') {
			...
			*verified = FALSE;
			printf(" FAILURE: %2d  %20.13e%s%s\n",
					m,xcr[m],quad_str[1],quad_str[2]);
		} else {
			printf("          %2d  %20.13e%s%se\n",
					m,xcr[m],quad_str[1],quad_str[2]);
		}
	}
	...
	quadmath_snprintf(quad_str[1], sizeof quad_str[1], "%20.13Qe", xciref);
	quadmath_snprintf(quad_str[2], sizeof quad_str[2], "%20.13Qe", xcidif);
