...
static void zran3(float ***z, int n1, int n2, int n3, int nx, int ny, int k);
static void showall(float ***z, int n1, int n2, int n3);
static double power( double a, int n );
static void bubble( float ten[M][2], int j1[M][2], int j2[M][2],
...
int main(int argc, char *argv[]) {
	...
	if (problem_class != 'U') {
		...
		if ( fabs( (double)rnm2 - verify_value ) <= epsilon ) {
			verified = TRUE;
			printf(" VERIFICATION SUCCESSFUL\n");
			printf(" L2 Norm is %20.12e\n", rnm2);
			printf(" Error is   %20.12e\n", (double)rnm2 - verify_value);
		} else {
			verified = FALSE;
...
static void norm2u3(float ***r, int n1, int n2, int n3,
	...
	for (i3 = 1; i3 < n3-1; i3++) {
		for (i2 = 1; i2 < n2-1; i2++) {
			for (i1 = 1; i1 < n1-1; i1++) {
				s = s + static_cast<__float128>(r[i3][i2][i1]) * static_cast<__float128>(r[i3][i2][i1]);
				a = fabsq(static_cast<__float128>(r[i3][i2][i1]));
				if (a > tmp) tmp = a;
			}
		}
	}
