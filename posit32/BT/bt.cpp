...
static void matvec_sub(posit32 ablock[5][5], posit32 avec[5], posit32 bvec[5]);
static void matmul_sub(posit32 ablock[5][5], posit32 bblock[5][5],
		posit32 cblock[5][5]);
static void binvcrhs(posit32 lhs[5][5], posit32 c[5][5], posit32 r[5]);
static void binvrhs(posit32 lhs[5][5], posit32 r[5]);
...
static void error_norm(posit32 rms[5]) {
	...
	int i, j, k, m, d;
	posit32 xi, eta, zeta, u_exact[5], add;
	...
	for (m = 0; m < 5; m++) {
		for (d = 0; d <= 2; d++) {
			rms[m] = rms[m] / p32(grid_points[d]-2);
		}
		rms[m] = sqrt(rms[m]);
	}
}
...
static void exact_rhs(void) {
	...
	posit32 dtemp[5], xi, eta, zeta, dtpp;
...
static void initialize(void) {
	...
	posit32 xi, eta, zeta, Pface[2][3][5], Pxi, Peta, Pzeta, temp[5];
	...
	for (i = 0; i < grid_points[0]; i++) {
		...
		for (j = 0; j < grid_points[1]; j++) {
			...
			for (k = 0; k < grid_points[2]; k++) {
				...
				for (ix = 0; ix < 2; ix++) {
					exact_solution(p32(ix), eta, zeta, 
							&(Pface[ix][0][0]));
				}

				for (iy = 0; iy < 2; iy++) {
					exact_solution(xi, p32(iy) , zeta, 
							&Pface[iy][1][0]);
				}

				for (iz = 0; iz < 2; iz++) {
					exact_solution(xi, eta, p32(iz),   
							&Pface[iz][2][0]);
				}
...
static void compute_rhs(void) {
	...
	int i, j, k, m;
	posit32 rho_inv, uijk, up1, um1, vijk, vp1, vm1, wijk, wp1, wm1;
...
static void set_constants(void) {
	...
	dnxm1 = 1.0 / p32(grid_points[0]-1);
	dnym1 = 1.0 / p32(grid_points[1]-1);
	dnzm1 = 1.0 / p32(grid_points[2]-1);
...
static void verify(int no_time_steps, char *problem_class, boolean *verified) {
	...
	posit32 xcrref[5],xceref[5],xcrdif[5],xcedif[5], 
	...
	*verified = TRUE;
	...
	if (*problem_class != 'U') {
		...
		if (p32(fabs((dt-dtref).toDouble())) > epsilon) {
			*verified = FALSE;
			*problem_class = 'U';
