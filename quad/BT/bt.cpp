...
static void rhs_norm(__float128 rms[5]);
...
static void matvec_sub(__float128 ablock[5][5], __float128 avec[5], __float128 bvec[5]);
static void matmul_sub(__float128 ablock[5][5], __float128 bblock[5][5],
		__float128 cblock[5][5]);
static void binvcrhs(__float128 lhs[5][5], __float128 c[5][5], __float128 r[5]);
static void binvrhs(__float128 lhs[5][5], __float128 r[5]);
...
int main(int argc, char **argv) {
	...
	if (fp != NULL) {
		...
		dt = strtoflt128(quad_str[0], NULL);
		while (fgetc(fp) != '\n');
		fscanf(fp, "%d%d%d",
	...
	printf(" Size: %3dx%3dx%3d\n",
			grid_points[0], grid_points[1], grid_points[2]);
	printf(" Iterations: %3d   dt: %10.6Qf\n", niter, dt);
...
static void error_norm(__float128 rms[5]) {
	...
	for (i = 0; i < grid_points[0]; i++) {
		xi = (__float128)i * (__float128)dnxm1;
		for (j = 0; j < grid_points[1]; j++) {
			eta = (__float128)j * (__float128)dnym1;
			for (k = 0; k < grid_points[2]; k++) {
				zeta = (__float128)k * (__float128)dnzm1;
...
static void exact_rhs(void) {
	...
	__float128 dtemp[5], xi, eta, zeta, dtpp;
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
	for (j = 1; j < grid_points[1]-1; j++) {
		eta = (__float128)j * dnym1;

		for (k = 1; k < grid_points[2]-1; k++) {
			zeta = (__float128)k * dnzm1;

			for (i = 0; i < grid_points[0]; i++) {
				xi = (__float128)i * dnxm1;
...
static void initialize(void) {
	...
	__float128 xi, eta, zeta, Pface[2][3][5], Pxi, Peta, Pzeta, temp[5];
	...
	for (i = 0; i < grid_points[0]; i++) {
		...
		for (j = 0; j < grid_points[1]; j++) {
			...
			for (k = 0; k < grid_points[2]; k++) {
				...
				for (ix = 0; ix < 2; ix++) {
					exact_solution((__float128)ix, eta, zeta, 
							&(Pface[ix][0][0]));
				}

				for (iy = 0; iy < 2; iy++) {
					exact_solution(xi, (__float128)iy , zeta, 
							&Pface[iy][1][0]);
				}

				for (iz = 0; iz < 2; iz++) {
					exact_solution(xi, eta, (__float128)iz,   
							&Pface[iz][2][0]);
				}
...
static void compute_rhs(void) {
	...
	int i, j, k, m;
	__float128 rho_inv, uijk, up1, um1, vijk, vp1, vm1, wijk, wp1, wm1;
...
static void set_constants(void) {
	...
	dnxm1 = 1.0 / (__float128)(grid_points[0]-1);
	dnym1 = 1.0 / (__float128)(grid_points[1]-1);
	dnzm1 = 1.0 / (__float128)(grid_points[2]-1);
...
static void verify(int no_time_steps, char *problem_class, boolean *verified) {
	...
	__float128 xcrref[5],xceref[5],xcrdif[5],xcedif[5], 
	...
	*verified = TRUE;
	...
	if (*problem_class != 'U') {
		...
		if (fabsq(dt-dtref) > epsilon) {
			*verified = FALSE;
			*problem_class = 'U';
			printf(" DT does not match the reference value of %15.8Qe\n", dtref);
		}
	} else {
