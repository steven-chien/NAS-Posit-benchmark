...
#include "compute_residual.h"	/* must be before npbparams, no include guard there */
#include "npb-C.h"
#include "npbparams.h"
...
/* common /main_flt_mem/ */
static __float128 v[NA+1+1];	/* v[1:NA+1] */
static __float128 aelt[NZ+1];	/* aelt[1:NZ] */
static __float128 a[NZ+1];		/* a[1:NZ] */
static __float128 x[NA+2+1];	/* x[1:NA+2] */
static __float128 z[NA+2+1];	/* z[1:NA+2] */
static __float128 p[NA+2+1];	/* p[1:NA+2] */
static __float128 q[NA+2+1];	/* q[1:NA+2] */
static __float128 r[NA+2+1];	/* r[1:NA+2] */
...
static void conj_grad (int colidx[], int rowstr[], __float128 x[], __float128 z[],
		__float128 a[], __float128 p[], __float128 q[], __float128 r[],
		//double w[],
		__float128 *rnorm);
static void makea(int n, int nz, __float128 a[], int colidx[], int rowstr[],
		int nonzer, int firstrow, int lastrow, int firstcol,
		int lastcol, __float128 rcond, int arow[], int acol[],
		__float128 aelt[], __float128 v[], int iv[], __float128 shift );
static void sparse(__float128 a[], int colidx[], int rowstr[], int n,
		int arow[], int acol[], __float128 aelt[],
		int firstrow, int lastrow,
		__float128 x[], boolean mark[], int nzloc[], int nnza);
static void sprnvc(int n, int nz, __float128 v[], int iv[], int nzloc[],
		int mark[]);
static int icnvrt(double x, int ipwr2);
static void vecset(int n, __float128 v[], int iv[], int *nzv, int i, __float128 val);
...
int main(int argc, char **argv) {
	...
	__float128 zeta;
	__float128 rnorm;
	__float128 norm_temp11;
	__float128 norm_temp12;
	double t, mflops;
	char problem_class;
	boolean verified;
	__float128 zeta_verify_value, epsilon;
	...
	for (it = 1; it <= 1; it++) {
		...
		norm_temp12 = 1.0q / sqrtq( norm_temp12 );
	...
	for (it = 1; it <= NITER; it++) {
		...
		zeta = (__float128)SHIFT + 1.0q / norm_temp11;

		if( it == 1 ) {
			printf("   iteration           ||r||                 zeta        |zeta - zeta_truth|\n");
		}

		printf("    %5d       %20.14Qe%20.13Qe%20.13Qe\n", it, rnorm, zeta, fabsq(zeta-zeta_verify_value));
	...
	epsilon = 1.0e-10;
	if (problem_class != 'U') {
		if (fabsq(zeta - zeta_verify_value) <= epsilon) {
			verified = TRUE;
			printf(" VERIFICATION SUCCESSFUL\n");
			printf(" Zeta is    %20.12Qe\n", zeta);
			printf(" Error is   %20.12Qe\n", zeta - zeta_verify_value);
		} else {
			verified = FALSE;
			printf(" VERIFICATION FAILED\n");
			printf(" Zeta                %20.12Qe\n", zeta);
			printf(" The correct zeta is %20.12Qe\n", zeta_verify_value);
		}
	} else {
		verified = FALSE;
  ...
  c---------------------------------------------------------------------*/
{
	...
	__float128 d, sum, rho, rho0, alpha, beta;
	int i, j, k;
	int cgit, cgitmax = 25;
	...
	for (cgit = 1; cgit <= cgitmax; cgit++) {
		...
		if(callcount>25) {
			compute_residual<__float128>(callcount-25, a, z, x, NA, firstrow, lastrow, firstcol, lastcol, rowstr, colidx);
		}
...
static void makea(
		...
		int arow[],		/* arow[1:nz] */
		int acol[],		/* acol[1:nz] */
		__float128 aelt[],	/* aelt[1:nz] */
		__float128 v[],		/* v[1:n+1] */
		int iv[],		/* iv[1:2*n+1] */
		__float128 shift )
{
	...
	__float128 size, ratio, scale;
	int jcol;

	size = 1.0q;
	ratio = powq(rcond, (1.0q / (__float128)n));
	nnza = 0;
