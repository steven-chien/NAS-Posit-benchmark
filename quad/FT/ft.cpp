...
static void evolve(qcomplex u0[NZ][NY][NX], qcomplex u1[NZ][NY][NX],
		int t, int indexmap[NZ][NY][NX], int d[3]);
static void compute_initial_conditions(qcomplex u0[NZ][NY][NX], int d[3]);
...
static void fft(int dir, qcomplex x1[NZ][NY][NX], qcomplex x2[NZ][NY][NX]);
static void cffts1(int is, int d[3], qcomplex x[NZ][NY][NX],
		qcomplex xout[NZ][NY][NX],
		qcomplex y0[NX][FFTBLOCKPAD],
		qcomplex y1[NX][FFTBLOCKPAD]);
static void cffts2(int is, int d[3], qcomplex x[NZ][NY][NX],
		qcomplex xout[NZ][NY][NX],
		qcomplex y0[NX][FFTBLOCKPAD],
		qcomplex y1[NX][FFTBLOCKPAD]);
static void cffts3(int is, int d[3], qcomplex x[NZ][NY][NX],
		qcomplex xout[NZ][NY][NX],
		qcomplex y0[NX][FFTBLOCKPAD],
		qcomplex y1[NX][FFTBLOCKPAD]);
static void fft_init (int n);
static void cfftz (int is, int m, int n, qcomplex x[NX][FFTBLOCKPAD],
		qcomplex y[NX][FFTBLOCKPAD]);
static void fftz2 (int is, int l, int m, int n, int ny, int ny1,
		qcomplex u[NX], qcomplex x[NX][FFTBLOCKPAD],
		qcomplex y[NX][FFTBLOCKPAD]);
static int ilog2(int n);
static void checksum(int i, qcomplex u1[NZ][NY][NX], int d[3]);
...
int main(int argc, char **argv) {
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
	static qcomplex u0[NZ][NY][NX];
	static qcomplex pad1[3];
	static qcomplex u1[NZ][NY][NX];
	static qcomplex pad2[3];
	static qcomplex u2[NZ][NY][NX];
	static qcomplex pad3[3];
	...
	static qcomplex truth[NZ][NY][NX];
	...
	compute_error<qcomplex, qcomplex>(&u1[0][0][0], &truth[0][0][0], NZ*NY*NX);
	...
	fft(1, u1, u0);
	save_array<qcomplex>("./../FT_truth", "FT", "quad", problem_class, 0, &(u0[0][0][0]), NZ*NY*NX);
	if (TIMERS_ENABLED == TRUE) {   
		timer_stop(T_FFT);
	}
	...
	for (iter = 1; iter <= niter; iter++) {
		...
		if(iter==1)
			save_array<qcomplex>("./../FT_truth", "FT", "quad", problem_class, 1, &(u1[0][0][0]), NZ*NY*NX);
		if (TIMERS_ENABLED == TRUE) {  
			timer_start(T_FFT);
		}
		...
		if(iter==1)
			save_array<qcomplex>("./../FT_truth", "FT", "quad", problem_class, 2, &(u2[0][0][0]), NZ*NY*NX);
	}
	...
	c_print_results("FT", problem_class, NX, NY, NZ, niter, nthreads,
			total_time, mflops, "     quad floating point", verified, 
...
static void fftz2 (int is, int l, int m, int n, int ny, int ny1,
	...
	for (i = 0; i < li; i++) {
		...
		for (k = 0; k < lk; k++) {
			for (j = 0; j < ny; j++) {
				__float128 x11real, x11imag;
				__float128 x21real, x21imag;
...
static void checksum(int i, qcomplex u1[NZ][NY][NX], int d[3]) {
	...
	/* complex % real */
	sums[i].real = sums[i].real/(__float128)(NTOTAL);
	sums[i].imag = sums[i].imag/(__float128)(NTOTAL);

	printf("T = %5d     Checksum = %22.12Qe %22.12Qe\n",
...
static void verify (int d1, int d2, int d3, int nt,
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
	__float128 vdata_real_s[6+1] = { 0.0,
		5.546087004964e+02,
		5.546385409189e+02,
		5.546148406171e+02,
		5.545423607415e+02,
		5.544255039624e+02,
		5.542683411902e+02 };
	__float128 vdata_imag_s[6+1] = { 0.0,
		4.845363331978e+02,
		4.865304269511e+02,
		4.883910722336e+02,
		4.901273169046e+02,
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
	__float128 vdata_real_w[6+1] = { 0.0,
		5.673612178944e+02,
		5.631436885271e+02,
		5.594024089970e+02,
		5.560698047020e+02,
		5.530898991250e+02,
		5.504159734538e+02 };
	__float128 vdata_imag_w[6+1] = { 0.0,
		5.293246849175e+02,
		5.282149986629e+02,
		5.270996558037e+02, 
		5.260027904925e+02, 
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
	__float128 vdata_real_a[6+1] = { 0.0,
		5.046735008193e+02,
		5.059412319734e+02,
		5.069376896287e+02,
		5.077892868474e+02,
		5.085233095391e+02,
		5.091487099959e+02 };
	__float128 vdata_imag_a[6+1] = { 0.0,
		5.114047905510e+02,
		5.098809666433e+02,
		5.098144042213e+02,
		5.101336130759e+02,
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
	__float128 vdata_real_b[20+1] = { 0.0,
		5.177643571579e+02,
		5.154521291263e+02,
		5.146409228649e+02,
		5.142378756213e+02,
		...
		5.124551951846e+02,
		5.124146770029e+02 };
	__float128 vdata_imag_b[20+1] = { 0.0,
		5.077803458597e+02,
		5.088249431599e+02,                  
		5.096208912659e+02,
		5.101023387619e+02,                  
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
	__float128 vdata_real_c[20+1] = { 0.0,
		5.195078707457e+02,
		5.155422171134e+02,
		5.144678022222e+02,
		5.140150594328e+02,
		...
		5.129905029333e+02,
		5.129714421109e+02 };
	__float128 vdata_imag_c[20+1] = { 0.0,
		5.149019699238e+02,
		5.127578201997e+02,
		5.122251847514e+02,
		5.121090289018e+02,
