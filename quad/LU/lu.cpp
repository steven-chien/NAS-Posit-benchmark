...
char quadmath_str[32768];

/* function declarations */
static void blts (int nx, int ny, int nz, int k,
		__float128 omega,
		__float128 v[ISIZ1][ISIZ2/2*2+1][ISIZ3/2*2+1][5],
		__float128 ldz[ISIZ1][ISIZ2][5][5],
		__float128 ldy[ISIZ1][ISIZ2][5][5],
		__float128 ldx[ISIZ1][ISIZ2][5][5],
		__float128 d[ISIZ1][ISIZ2][5][5],
		int ist, int iend, int jst, int jend,
		int nx0, int ny0 );
static void buts(int nx, int ny, int nz, int k,
		__float128 omega,
		__float128 v[ISIZ1][ISIZ2/2*2+1][ISIZ3/2*2+1][5],
		__float128 tv[ISIZ1][ISIZ2][5],
		__float128 d[ISIZ1][ISIZ2][5][5],
		__float128 udx[ISIZ1][ISIZ2][5][5],
		__float128 udy[ISIZ1][ISIZ2][5][5],
		__float128 udz[ISIZ1][ISIZ2][5][5],
		int ist, int iend, int jst, int jend,
		int nx0, int ny0 );
...
static void verify(__float128 xcr[5], __float128 xce[5], __float128 xci,
...
static void erhs(void) {
	...
	__float128  q;
	__float128  u21, u31, u41;
	__float128  tmp;
	__float128  u21i, u31i, u41i, u51i;
	__float128  u21j, u31j, u41j, u51j;
	__float128  u21k, u31k, u41k, u51k;
	__float128  u21im1, u31im1, u41im1, u51im1;
	__float128  u21jm1, u31jm1, u41jm1, u51jm1;
	__float128  u21km1, u31km1, u41km1, u51km1;
	...
	for (i = 0; i < nx; i++) {
		iglob = i;
		xi = ( (__float128)(iglob) ) / ( nx0 - 1 );
		for (j = 0; j < ny; j++) {
			jglob = j;
			eta = ( (__float128)(jglob) ) / ( ny0 - 1 );
			for (k = 0; k < nz; k++) {
				zeta = ( (__float128)(k) ) / ( nz - 1 );
				for (m = 0; m < 5; m++) {
					rsd[i][j][k][m] =  ce[m][0]
						+ ce[m][1] * xi
						+ ce[m][2] * eta
...
static void exact( int i, int j, int k, __float128 u000ijk[5] ) {
	...
	xi  = ((__float128)i) / (nx0 - 1);
	eta  = ((__float128)j) / (ny0 - 1);
	zeta = ((__float128)k) / (nz - 1);
...
static void pintgr(void) {
	...
	__float128 phi1[ISIZ2+2][ISIZ3+2];	/* phi1(0:isiz2+1,0:isiz3+1) */
	__float128 phi2[ISIZ2+2][ISIZ3+2];	/* phi2(0:isiz2+1,0:isiz3+1) */
	__float128  frc1, frc2, frc3;
...
static void read_input(void) {
	...
	if (fp != NULL) {
		...
		while(fgetc(fp) != '\n'); while(fgetc(fp) != '\n');
		fscanf(fp, "%s", quadmath_str);
		dt = strtoflt128(quadmath_str, NULL);
		//fscanf(fp, "%lf", &dt);
		while(fgetc(fp) != '\n');

		while(fgetc(fp) != '\n'); while(fgetc(fp) != '\n');
		fscanf(fp, "%s", quadmath_str);
		omega = strtoflt128(quadmath_str, NULL);
		//fscanf(fp, "%lf", &omega);
		while(fgetc(fp) != '\n');
...
static void setiv(void) {
	...
	int iglob, jglob;
	__float128  xi, eta, zeta;
	__float128  pxi, peta, pzeta;
	__float128  ue_1jk[5],ue_nx0jk[5],ue_i1k[5],
...
static void ssor(void) {
	...
	int istep;
	__float128  tmp;
	__float128  delunm[5], tv[ISIZ1][ISIZ2][5];
...
static void verify(__float128 xcr[5], __float128 xce[5], __float128 xci,
	...
	*verified = TRUE;
	...
	for (m = 0; m < 5; m++) {
		...
		xcrdif[m] = fabsq((xcr[m]-xcrref[m])/xcrref[m]);
		xcedif[m] = fabsq((xce[m]-xceref[m])/xceref[m]);

	}
	xcidif = fabsq((xci - xciref)/xciref);
	...
	if (*problem_class != 'U') {
		...
		printf(" Accuracy setting for epsilon = %20.13Qe\n", epsilon);
		if (fabsq(dt-dtref) > epsilon) {  
			*verified = FALSE;
			*problem_class = 'U';
			printf(" DT does not match the reference value of %20.13Qe\n", dtref);
		}
	} else {
	...
	for (m = 0; m < 5; m++) {
		if (*problem_class  ==  'U') {
			...
			*verified = FALSE;
			printf(" FAILURE: %2d  %20.13Qe%20.13Qe%20.13Qe\n",
					m, xcr[m], xcrref[m], xcrdif[m]);
		} else {
			printf("          %2d  %20.13Qe%20.13Qe%20.13Qe\n",
					m, xcr[m], xcrref[m], xcrdif[m]);
		}
	}
