...
static void mg3P(__float128 ****u, __float128 ***v, __float128 ****r, __float128 a[4],
		__float128 c[4], int n1, int n2, int n3, int k);
static void psinv( __float128 ***r, __float128 ***u, int n1, int n2, int n3,
		__float128 c[4], int k);
static void resid( __float128 ***u, __float128 ***v, __float128 ***r,
		int n1, int n2, int n3, __float128 a[4], int k );
static void rprj3( __float128 ***r, int m1k, int m2k, int m3k,
		__float128 ***s, int m1j, int m2j, int m3j, int k );
static void interp( __float128 ***z, int mm1, int mm2, int mm3,
		__float128 ***u, int n1, int n2, int n3, int k );
static void norm2u3(__float128 ***r, int n1, int n2, int n3,
		__float128 *rnm2, __float128 *rnmu, int nx, int ny, int nz);
static void rep_nrm(__float128 ***u, int n1, int n2, int n3,
		char *title, int kk);
static void comm3(__float128 ***u, int n1, int n2, int n3, int kk);
static void zran3(__float128 ***z, int n1, int n2, int n3, int nx, int ny, int k);
static void showall(__float128 ***z, int n1, int n2, int n3);
static double power( double a, int n );
static void bubble( __float128 ten[M][2], int j1[M][2], int j2[M][2],
		int j3[M][2], int m, int ind );
static void zero3(__float128 ***z, int n1, int n2, int n3);
static void nonzero(__float128 ***z, int n1, int n2, int n3);
...
int main(int argc, char *argv[]) {
	...
	__float128 rnm2, rnmu;
	__float128 epsilon = 1.0e-8;
	int n1, n2, n3, nit;
	__float128 verify_value;
	boolean verified;
	...
	u = (__float128 ****)malloc((lt+1)*sizeof(__float128 ***));
	for (l = lt; l >=1; l--) {
		u[l] = (__float128 ***)malloc(m3[l]*sizeof(__float128 **));
		for (k = 0; k < m3[l]; k++) {
			u[l][k] = (__float128 **)malloc(m2[l]*sizeof(__float128 *));
			for (j = 0; j < m2[l]; j++) {
				u[l][k][j] = (__float128 *)malloc(m1[l]*sizeof(__float128));
			}
		}
	}
	v = (__float128 ***)malloc(m3[lt]*sizeof(__float128 **));
	for (k = 0; k < m3[lt]; k++) {
		v[k] = (__float128 **)malloc(m2[lt]*sizeof(__float128 *));
		for (j = 0; j < m2[lt]; j++) {
			v[k][j] = (__float128 *)malloc(m1[lt]*sizeof(__float128));
		}
	}
	r = (__float128 ****)malloc((lt+1)*sizeof(__float128 ***));
	for (l = lt; l >=1; l--) {
		r[l] = (__float128 ***)malloc(m3[l]*sizeof(__float128 **));
		for (k = 0; k < m3[l]; k++) {
			r[l][k] = (__float128 **)malloc(m2[l]*sizeof(__float128 *));
			for (j = 0; j < m2[l]; j++) {
				r[l][k][j] = (__float128 *)malloc(m1[l]*sizeof(__float128));
			}
		}
	}
	...
	if (problem_class != 'U') {
		...
		if ( fabsq( rnm2 - verify_value ) <= epsilon ) {
			verified = TRUE;
			printf(" VERIFICATION SUCCESSFUL\n");
			printf(" L2 Norm is %20.12Qe\n", rnm2);
			printf(" Error is   %20.12Qe\n", rnm2 - verify_value);
		} else {
			verified = FALSE;
			printf(" VERIFICATION FAILED\n");
			printf(" L2 Norm is             %20.12Qe\n", rnm2);
			printf(" The correct L2 Norm is %20.12Qe\n", verify_value);
		}
	} else {
		verified = FALSE;
...
static void rep_nrm(__float128 ***u, int n1, int n2, int n3,
	...
	printf(" Level%2d in %8s: norms =%20.12Qe%20.12Qe\n",
			kk, title, rnm2, rnmu);
}
