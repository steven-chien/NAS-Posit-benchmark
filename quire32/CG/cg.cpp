...
/* common /main_flt_mem/ */
static posit32 v[NA+1+1];	/* v[1:NA+1] */
static posit32 aelt[NZ+1];	/* aelt[1:NZ] */
static posit32 a[NZ+1];		/* a[1:NZ] */
static posit32 x[NA+2+1];	/* x[1:NA+2] */
static posit32 z[NA+2+1];	/* z[1:NA+2] */
static posit32 p[NA+2+1];	/* p[1:NA+2] */
static posit32 q[NA+2+1];	/* q[1:NA+2] */
static posit32 r[NA+2+1];	/* r[1:NA+2] */
...
static void conj_grad (int colidx[], int rowstr[], posit32 x[], posit32 z[],
		posit32 a[], posit32 p[], posit32 q[], posit32 r[],
		//double w[],
		posit32 *rnorm);
static void makea(int n, int nz, posit32 a[], int colidx[], int rowstr[],
		int nonzer, int firstrow, int lastrow, int firstcol,
		int lastcol, posit32 rcond, int arow[], int acol[],
		posit32 aelt[], posit32 v[], int iv[], posit32 shift );
static void sparse(posit32 a[], int colidx[], int rowstr[], int n,
		int arow[], int acol[], posit32 aelt[],
		int firstrow, int lastrow,
		posit32 x[], boolean mark[], int nzloc[], int nnza);
static void sprnvc(int n, int nz, posit32 v[], int iv[], int nzloc[],
		int mark[]);
static int icnvrt(double x, int ipwr2);
static void vecset(int n, posit32 v[], int iv[], int *nzv, int i, posit32 val);
  ...
  c---------------------------------------------------------------------*/
{
	...
	//posit32 d, sum, rho, rho0, alpha, beta;
	posit32 alpha, beta;
	quire32 d, sum, rho, rho0;
	int i, j, k;
	int cgit, cgitmax = 25;
	...
	for (cgit = 1; cgit <= cgitmax; cgit++) {
		...
		if(callcount>25) {
			compute_residual<posit32>(callcount-25, a, z, x, NA, firstrow, lastrow, firstcol, lastcol, rowstr, colidx);
		}
	...
	for (j = 1; j <= lastcol-firstcol+1; j++) {
		//d = x[j] - r[j];
		//sum = sum + d*d;
		posit32 tmp_d = x[j] - r[j];
		sum = sum.qma(tmp_d, tmp_d);
	}
	(*rnorm) = sum.toPosit().sqrt();
}
