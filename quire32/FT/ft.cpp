...
int main(int argc, char **argv) {
	...
	c_print_results("FT", problem_class, NX, NY, NZ, niter, nthreads,
			total_time, mflops, "  posit32 floating point", verified, 
...
static void fftz2 (int is, int l, int m, int n, int ny, int ny1,
	...
	for (i = 0; i < li; i++) {
		...
		for (k = 0; k < lk; k++) {
			for (j = 0; j < ny; j++) {
				quire32 sum;
				posit32 x11real, x11imag;
				posit32 x21real, x21imag;
				...
				sum = sum.clr();
				sum = sum.qma(u1.real, x11real - x21real);
				sum = sum.qms(u1.imag, x11imag - x21imag);
				y[i22+k][j].real = sum.toPosit();
				//y[i22+k][j].real = u1.real * (x11real - x21real)
				//	- u1.imag * (x11imag - x21imag);

				sum = sum.clr();
				sum = sum.qma(u1.real, x11imag - x21imag);
				sum = sum.qma(u1.imag, x11real - x21real);
				y[i22+k][j].imag = sum.toPosit();
