...
static void norm2u3(posit32 ***r, int n1, int n2, int n3,
		__float128 *rnm2, __float128 *rnmu, int nx, int ny, int nz);

/* functions prototypes */
static void setup(int *n1, int *n2, int *n3, int lt);
static void mg3P(posit32 ****u, posit32 ***v, posit32 ****r, posit32 a[4],
		posit32 c[4], int n1, int n2, int n3, int k);
static void psinv( posit32 ***r, posit32 ***u, int n1, int n2, int n3,
		posit32 c[4], int k);
static void resid( posit32 ***u, posit32 ***v, posit32 ***r,
		int n1, int n2, int n3, posit32 a[4], int k );
static void rprj3( posit32 ***r, int m1k, int m2k, int m3k,
		posit32 ***s, int m1j, int m2j, int m3j, int k );
static void interp( posit32 ***z, int mm1, int mm2, int mm3,
		posit32 ***u, int n1, int n2, int n3, int k );
static void norm2u3(posit32 ***r, int n1, int n2, int n3,
		posit32 *rnm2, posit32 *rnmu, int nx, int ny, int nz);
static void rep_nrm(posit32 ***u, int n1, int n2, int n3,
		char *title, int kk);
static void comm3(posit32 ***u, int n1, int n2, int n3, int kk);
static void zran3(posit32 ***z, int n1, int n2, int n3, int nx, int ny, int k);
static void showall(posit32 ***z, int n1, int n2, int n3);
static double power( double a, int n );
static void bubble( posit32 ten[M][2], int j1[M][2], int j2[M][2],
		int j3[M][2], int m, int ind );
static void zero3(posit32 ***z, int n1, int n2, int n3);
static void nonzero(posit32 ***z, int n1, int n2, int n3);
...
int main(int argc, char *argv[]) {
	...
	u = (posit32 ****)malloc((lt+1)*sizeof(posit32 ***));
	for (l = lt; l >=1; l--) {
		u[l] = (posit32 ***)malloc(m3[l]*sizeof(posit32 **));
		for (k = 0; k < m3[l]; k++) {
			u[l][k] = (posit32 **)malloc(m2[l]*sizeof(posit32 *));
			for (j = 0; j < m2[l]; j++) {
				u[l][k][j] = (posit32 *)malloc(m1[l]*sizeof(posit32));
			}
		}
	}
	v = (posit32 ***)malloc(m3[lt]*sizeof(posit32 **));
	for (k = 0; k < m3[lt]; k++) {
		v[k] = (posit32 **)malloc(m2[lt]*sizeof(posit32 *));
		for (j = 0; j < m2[lt]; j++) {
			v[k][j] = (posit32 *)malloc(m1[lt]*sizeof(posit32));
		}
	}
	r = (posit32 ****)malloc((lt+1)*sizeof(posit32 ***));
	for (l = lt; l >=1; l--) {
		r[l] = (posit32 ***)malloc(m3[l]*sizeof(posit32 **));
		for (k = 0; k < m3[l]; k++) {
			r[l][k] = (posit32 **)malloc(m2[l]*sizeof(posit32 *));
			for (j = 0; j < m2[l]; j++) {
				r[l][k][j] = (posit32 *)malloc(m1[l]*sizeof(posit32));
			}
		}
	}
...
static void psinv( posit32 ***r, posit32 ***u, int n1, int n2, int n3,
	...
	for (i3 = 1; i3 < n3-1; i3++) {
		for (i2 = 1; i2 < n2-1; i2++) {
			...
			for (i1 = 1; i1 < n1-1; i1++) {
				quire32 sum;
				sum = sum.qma(1.0, u[i3][i2][i1]);
				sum = sum.qma(c[0], r[i3][i2][i1]);
				sum = sum.qma(c[1], r[i3][i2][i1-1] + r[i3][i2][i1+1] + r1[i1]);
				sum = sum.qma(c[2], r2[i1] + r1[i1-1] + r1[i1+1]);
				u[i3][i2][i1] = sum.toPosit();
	...
	for (i3 = 1; i3 < n3-1; i3++) {
		for (i2 = 1; i2 < n2-1; i2++) {
			...
			for (i1 = 1; i1 < n1-1; i1++) {
				...
				quire32 sum;
				sum = sum.qma(1.0, v[i3][i2][i1]);
				sum = sum.qms(a[0], u[i3][i2][i1]);
				sum = sum.qms(a[2], u2[i1] + u1[i1-1] + u1[i1+1]);
				sum = sum.qms(a[3], u2[i1-1] + u2[i1+1]);
				r[i3][i2][i1] = sum.toPosit();
			}
		}
	}
	...
	for (j3 = 1; j3 < m3j-1; j3++) {
		...
		for (j2 = 1; j2 < m2j-1; j2++) {
			...
			for (j1 = 1; j1 < m1j-1; j1++) {
				...
				quire32 sum;
				sum = sum.qma(0.5, r[i3+1][i2+1][i1+1]);
				sum = sum.qma(0.25, r[i3+1][i2+1][i1] + r[i3+1][i2+1][i1+2] + x2);
				sum = sum.qma(0.125, x1[i1] + x1[i1+2] + y2);
				sum = sum.qma(0.0625, y1[i1] + y1[i1+2]);
				s[j3][j2][j1] = sum.toPosit();
			}
		}
	}
